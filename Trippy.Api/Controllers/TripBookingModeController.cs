...
namespace Trippy.Api.Controllers
{
    ...
    public class TripBookingModeController : Api_BaseController
    {
        ...
        {
            var response = new CustomApiResponse();
            var tripBookingMode = await _service.GetByIdAsync(id);
            if (tripBookingMode == null)
            {
                response.IsSucess = false;
            ...
            {
                response.IsSucess = true;
                response.Value = tripBookingMode;
                response.StatusCode = 200;
            }
            return response;
        }
        [HttpPost]
        public async Task<CustomApiResponse> Create([FromBody] TripBookingMode tripBookingMode)
        {
            var response = new CustomApiResponse();
            try
            {
                var created = await _service.CreateAsync(tripBookingMode);
                response.IsSucess = true;
        ...
        [HttpPut("{id}")]
        public async Task<CustomApiResponse> Update(int id, [FromBody] TripBookingMode tripBookingMode)
        {
            var response = new CustomApiResponse();
            if (id != tripBookingMode.TripBookingModeId)
            {
                response.IsSucess = false;
            ...
            var updated = await _service.UpdateAsync(tripBookingMode);
            if (!updated)
            {
                response.IsSucess = false;
                response.Error = "Not found";
